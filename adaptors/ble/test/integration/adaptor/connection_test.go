	...
	Context("Server", func() {
		...
		It("should process the input device", func() {
			...
			mockServer.EXPECT().Recv().Return(&v1alpha1.ConnectRequest{
				...
				Device: []byte(`{"apiVersion":"edge.cattle.io/v1alpha1","kind":"DeviceLink","metadata":{"name":"xiaomi-temp-rs2200", "namespace":"default"},"spec":{"template":{"spec":{"protocol":{"name":"MJ_HT_V1","macAddress":""},"properties":[{"name":"data","description":"XiaoMi temp sensor with temperature and humidity data","accessMode":"NotifyOnly","visitor":{"characteristicUUID":"226c000064764566756266734470666d"}}]}}}}
`),
			}, nil)
